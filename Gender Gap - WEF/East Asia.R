...
east_asia <- c("China", "Japan", "Korea, Rep.", "Mongolia")
...
overall[which((overall$Country %in% east_asia)==T),5] <- "#867A72"
...
eco_par <- eco_par[which((eco_par$Country %in% east_asia)==T),]
overall <- overall[which((overall$Country %in% east_asia)==T),]
edu_att <- edu_att[which((edu_att$Country %in% east_asia)==T),]
income <- income[which((income$Country %in% east_asia)==T),]
head_state <- head_state[which((head_state$Country %in% east_asia)==T),]
health <- health[which((health$Country %in% east_asia)==T),]
life <- life[which((life$Country %in% east_asia)==T),]
LFP <- LFP[which((LFP$Country %in% east_asia)==T),]
literacy <- literacy[which((literacy$Country %in% east_asia)==T),]
LOM <- LOM[which((LOM$Country %in% east_asia)==T),]
minister <- minister[which((minister$Country %in% east_asia)==T),]
poli_emp <- poli_emp[which((poli_emp$Country %in% east_asia)==T),]
primary <- primary[which((primary$Country %in% east_asia)==T),]
pro_tech <- pro_tech[which((pro_tech$Country %in% east_asia)==T),]
secondary <- secondary[which((secondary$Country %in% east_asia)==T),]
sex_ratio <- sex_ratio[which((sex_ratio$Country %in% east_asia)==T),]
tertiary <- tertiary[which((tertiary$Country %in% east_asia)==T),]
wage <- wage[which((wage$Country %in% east_asia)==T),]
parliament <- parliament[which((parliament$Country %in% east_asia)==T),]
